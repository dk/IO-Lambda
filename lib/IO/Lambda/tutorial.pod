...
so-called "raw" mode.  This trick is used by so many programs, that standard
libraries like I<libtermcap>, I<libcurses>, and the newer I<libncurses>
emerged, to provide per-character input in a portable way.  Perl itself has no
API dedicated to raw input. Additional modules L<Curses>,
